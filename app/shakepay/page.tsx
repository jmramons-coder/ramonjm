import { PrivateStudyAnalytics } from "../analytics/analytics";
import { HugeiconsIcon } from "@hugeicons/react";
import { ArrowLeft02Icon, PlayIcon, Mail01Icon } from "@hugeicons/core-free-icons";
...
export const metadata: Metadata = { title:"Private product exploration — Ramon JM", description:"An independent product design study, shared for discussion.", robots:{index:false,follow:false}, alternates:{canonical:"/shakepay"}, openGraph:{title:"Private product exploration — Ramon JM",description:"Shared for discussion.",images:[]},twitter:{title:"Private product exploration",description:"Shared for discussion.",images:[]} };
export default async function Study({searchParams}:{searchParams:Promise<{error?:string}>}) {
 const access=await hasAccess(); const {error}=await searchParams;
 return <>{access && <PrivateStudyAnalytics />}{access && <SiteHeader textOnly backHref="/#explorations" backLabel="Back to portfolio" />}<ScrollBackground hasFooter={access} /><main className={styles.page} id="main-content">
 {!access ? <section className={styles.gate}><Link className={styles.back} href="/#explorations"><HugeiconsIcon icon={ArrowLeft02Icon} size={19} strokeWidth={1.8} aria-hidden="true" />Back to portfolio</Link><div className={styles.gateInner}><Image src="/shakepay-icon.png" alt="Shakepay app icon" width={64} height={64} className={styles.brandIcon}/><p className={styles.eyebrow}>Private product exploration</p><h1>A closer look.<br/><em>By invitation.</em></h1><p>A self-initiated exploration of rewards clarity in Shakepay, shared privately for feedback and discussion.</p><form action={unlock}><label htmlFor="code">Access code</label><input id="code" name="code" type="password" required maxLength={128} autoComplete="off" aria-describedby={error ? "access-error" : undefined}/>{error && <p id="access-error" role="alert">That code didn’t work. Please try again.</p>}<button type="submit">View the study</button></form><a className={styles.accessContact} href="mailto:jmanuelr.99@gmail.com?subject=Shakepay%20study%20access"><HugeiconsIcon icon={Mail01Icon} size={18} strokeWidth={1.8} aria-hidden="true" />Email Ramon for access</a><p className={styles.emailHint}>Opens your email app. Nothing is sent automatically.</p><small>Unsolicited study by Ramon JM. I am not a Shakepay employee, do not represent Shakepay, and was not commissioned or endorsed by the company.</small></div></section> : <>
 ...
 <section className={styles.closing}><p className={styles.eyebrow}>Built to start a conversation</p><h2>Small moments.<br/>A more complete picture.</h2><a className={styles.discussButton} href="mailto:jmanuelr.99@gmail.com?subject=Let’s%20discuss%20the%20rewards%20exploration">Let’s discuss the idea</a><form action={lock}><button>Lock this study</button></form><small>Film music: <a href="https://www.scottbuckley.com.au/library/origami/">Origami — Scott Buckley</a>, <a href="https://creativecommons.org/licenses/by/4.0/">CC BY 4.0</a>. Edited excerpt with added interaction sounds.</small></section>
 </> }</main></>;
}
